...
const ProjectDetailsPage = () => {
  ...
  const DONE_MAKER_API_URL =
    "https://konstruct.world/checklists/done-maker/";
  ...
  const getPhaseStorageKey = (pid, towerId) => `SELECTED_PHASE_${pid}_${towerId}`;
  ...
  const getActiveRoleFromStorage = (pid = projectId) => {
    ...
    const persistedRole = normalizeRole(Array.isArray(roles) ? roles[0] : roles);
  ...
  for (const r of results) {
    ...
    const available = Array.isArray(r?.available_for_me)
      ? r.available_for_me
      : [];
    const assigned = Array.isArray(r?.assigned_to_me)
      ? r.assigned_to_me
      : [];
                    ...
                    >
                      ...
                      >
                        <select
                          ...
                          onChange={(e) => handleTowerPhaseChange(proj.id, e.target.value)}
