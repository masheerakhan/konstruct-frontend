...
const [flatChecklistMap, setFlatChecklistMap] = useState({});
  ...
  const [floorInitProcessing, setFloorInitProcessing] = useState({});
...
//     ctx.items.some((i) => i.mode === "template" && i.template_id);
...
//   // ✅ Prevent duplicate calls for same level
// // ✅ prevent duplicate + parallel execution
// if (floorInitProcessing[levelId]) {
//   return floorInitContextMap[levelId] || {};
// }

// // if (floorInitContextMap[levelId]) {
// //   return floorInitContextMap[levelId];
// // }

// setFloorInitProcessing(prev => ({
//   ...prev,
//   [levelId]: true
// }));
...
// if (!stageId) {
//   console.warn("stage_id missing, skipping creation", ctx);
...
//     // // ✅ Create checklist for ALL template items
//     // for (const item of items) {
//     //   if (item.mode !== "template") continue;

//     //   try {
//     //     await NEWchecklistInstance.post(
//     //       "/create-live-checklist-from-template/",
//     //       {
//     //         template_id: item.template_id, // 🔥 REQUIRED
...
//         );
//       } catch (err) {
//         console.error("CHECKLIST_CREATE_FAILED", err);
//       }

//           // ✅ cache context after processing
//           setFloorInitContextMap((prev) => ({
//             ...prev,
//             [levelId]: ctx,
//           }));
...
//           return ctx;
//         } catch (error) {
//           console.error("INIT_CONTEXT_FAILED", {
//             levelId,
...
//   //   return;
//   // }
//   if (!resolvedProjectId || !resolvedPhaseId || !towerId) {
//   return;
// }

// if (!levels?.length) {
//   return; // wait for levels to load
// }
...
const ensureFloorChecklistForInitializer = async (levelId) => {
  ...
  try {
    ...
    // ✅ Only create when template exists
    ...
    // ✅ store error state (so UI / retry can handle)
    setFloorInitContextMap(prev => ({
      ...prev,
      [levelId]: { error: true },
    }));
...
useEffect(() => {
  ...
  // ✅ Non-initializer → skip init but allow fetch
  if (!isInitializerRole()) {
    setFloorInitReady(true);
    return;
  }
