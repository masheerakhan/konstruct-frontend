...
const Scheduling = () => {
  ...
  // ---- Project selection (from localStorage ACCESSES only) ----
  ...
  const fetchProjectName = async (id) => {
    if (!id || projectNameCache[id]) return `Project ${id}`;
    try {
      const { data } = await projectInstance.get(`/projects/${id}/`, {
        headers: { "Content-Type": "application/json" },
      });
      const name = data?.name || `Project ${id}`;
      setProjectNameCache((prev) => ({ ...prev, [id]: name }));
      return name;
    } catch {
      const fallback = `Project ${id}`;
      setProjectNameCache((prev) => ({ ...prev, [id]: fallback }));
      return fallback;
    }
  };

  useEffect(() => {
    const loadProjectsFromAccess = async () => {
      try {
        const raw = JSON.parse(localStorage.getItem("ACCESSES") || "[]");
        const accessList = Array.isArray(raw) ? raw : [];

        const uniqueMap = new Map();

        for (const access of accessList) {
          const pid = Number(access?.project_id);
          ...
          const localName =
            access?.project_name ||
            access?.project?.name ||
            projectNameCache[pid];

          if (localName) {
            uniqueMap.set(pid, { id: pid, name: localName });
          } else {
            uniqueMap.set(pid, { id: pid, name: `Project ${pid}` });
          }
        }

        const initialProjects = Array.from(uniqueMap.values()).sort((a, b) =>
          String(a.name).localeCompare(String(b.name))
        );
        setProjects(initialProjects);

        // fetch missing names in background
        const missing = initialProjects.filter(
          (p) => !p.name || p.name === `Project ${p.id}`
        );
        ...
        if (missing.length > 0) {
          const resolved = await Promise.all(
            missing.map(async (p) => ({
              id: p.id,
              name: await fetchProjectName(p.id),
            }))
          );

          const merged = new Map();
          initialProjects.forEach((p) => merged.set(p.id, p));
        ...
        if (!projectId && initialProjects.length === 1) {
          setActiveProject(initialProjects[0].id);
        } else if (
          projectId &&
          !initialProjects.some((p) => Number(p.id) === Number(projectId))
        ) {
          setActiveProject("");
        }
      } catch (e) {
        console.error("[Scheduling] loadProjectsFromAccess failed", e);
        setProjects([]);
        toast.error("Failed to load accessible projects");
      }
    };

    loadProjectsFromAccess();
  ...
  // ✅ project checklists
  const [allProjectChecklists, setAllProjectChecklists] = useState([]);
