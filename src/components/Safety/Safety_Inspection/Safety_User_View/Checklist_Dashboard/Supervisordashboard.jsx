...
export default function SupervisorDashboard() {
    ...
    const supervisorAssigned = asSupervisor.filter((c) => c.status === "in_progress");
    const supervisorPending = supervisorAssigned;
    const supervisorCompleted = asSupervisor.filter((c) => c.status === "completed");
    ...
    const handleInspectionSubmit = async () => {
        ...
        const missingRequiredPhoto = inspectionRows.find(
            ({ item, sub }) => item.photo_required && !(inspectorMedia[sub.id] || sub.has_photo)
        );
                                            ...
                                            >
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <PhotoUploadArea
                                                        ...
                                                        label={item.photo_required ? "Attach supervisor photo (required)" : "Attach supervisor photo (optional)"}
                        ...
                        {view === "dashboard" && (
                            <section>
                                ...
                                <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                                    {[
                                        { label: "Assigned", icon: <Clock className="h-3.5 w-3.5" />, color: "hsl(35, 90%, 50%)", count: supervisorAssigned.length },
                                        { label: "Pending", icon: <Eye className="h-3.5 w-3.5" />, color: "hsl(280, 70%, 50%)", count: supervisorPending.length },
                                        { label: "Completed", icon: <CheckCircle className="h-3.5 w-3.5" />, color: "hsl(145, 65%, 42%)", count: supervisorCompleted.length },
                                        { label: "Total", icon: <CalendarDays className="h-3.5 w-3.5" />, color: "hsl(220, 70%, 50%)", count: asSupervisor.length },
