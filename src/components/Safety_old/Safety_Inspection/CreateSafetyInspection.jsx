   ...
   useEffect(() => {
     ...
     (async () => {
       try {
         ...
         if (alive) setProjects(projectList);
         ...
         if (alive && active) {
           ...
           const selectedProject = projectList.find(
             (p) => String(p.id) === String(active),
           );

           const oid =
             selectedProject?.org ??
             selectedProject?.organization_id ??
             selectedProject?.organization?.id ??
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                console.log("Calling listSafetyCategories...");
    ...
    //                 const templateQuestions = Array.isArray(t?.questions)
    //                   ? t.questions
    //                   : [];

    //                 const qs = templateQuestions.map((q) => ({
            ...
            const qs = templateQuestions.map((q) => ({
              ...
              options: (Array.isArray(q?.options) ? q.options : []).map(
                (o, i) => ({
                  id: generateId(),
                  label:
                    typeof o === "string" ? o : (o?.label ?? `Option ${i + 1}`),
                }),
              ),
              photo_required: !!q.photo_required,
            }));
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <div className="bg-white border rounded-lg p-5">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Define the workflow. Choose role and user for each step. Manager
              can decide the order (e.g. Checker first, then Maker).
            </p>
            <div className="flex flex-col gap-4">
              {/* {movementSteps.map((step, idx) => ( */}
            {(Array.isArray(movementSteps) ? movementSteps : []).map((step, idx) => (
